...
async function incompleteData(name) {
  const response = await fetch(`https://www.omdbapi.com/?apikey=97753bae&s=${name}`);
...
async function fullData(imdbId) {
  const promises = imdbId.map(async (id) => {
    const response = await fetch(`https://www.omdbapi.com/?apikey=97753bae&i=${id}&plot=full`);
    const data = await response.json();
